      ...
      <div class="container">
        ...
        <section class="row content-header">
            <div class="header-title col-md-11">
              <h1 class="logo-caption"><i class="fa fa-handshake" style="color: #002f6d;"></i> <span class="tweak" style="color:#002f6d;">Prestataires</span></h1>
          ...
          <div class="col col-md-12 table-responsive">
            <div class="table-responsive">
            	<table class="table table-bordered table-striped table-hover">
            		<thead>
            			<tr>
                    <th style="width: 2%;">Id prestataire</th>
                    <th style="width: 10%;">Nom</th>
                   	<th style="width: 13%;">Prénom</th>
                    <th style="width: 13%;">Numéro</th>
                    <th style="width: 17%;">Ville</th>
                    <th style="width: 13%;">Métier</th>
                    <th style="width: 17%;">Age</th>
                    <th style="width: 17%;">Sexe</th>
                    <th style="width: 30%;">Action</th>
            			</tr>

                @foreach($prests as $prest)
                  <tr>
            				<th style="width: 2%;">{{ $prest->id }}</th>
                    <th style="width: 10%;">{{ $prest->nom_prest }}</th>
                   	<th style="width: 13%;">{{ $prest->prenom_prest }}</th>
                    <th style="width: 13%;">{{ $prest->contact_prest }}</th>
                    <th style="width: 17%;">{{ $prest->ville_prest }}</th>
                    <th style="width: 13%;">{{ $prest->metier_prest }}</th>
                    <th style="width: 17%;">{{ $prest->age_prest }}</th>
                    <th style="width: 17%;">{{ $prest->sexe_prest }}</th>
                    <th style="width: 30%;">
                      <!--button href="" class="btn btn-success btn-sm">
                         <i class="fa fa-check"></i>
                      </!--button-->
                      
                      @if($prest->deleted_at === null)
                          <form action="{{route('prestataire_destroy',$prest->id)}}" method="POST">
                            @csrf
                      ...
                      @else
                          <form action="{{route('prestataire_restore',$prest->id)}}" method="POST">
                              @csrf
                              <button type="submit" class="btn btn-secondary" style="margin:0px, 0px, 20px, 20px;">Restauré
                                   <!--i class="fa fa-close"></!--i-->
                              </button>
                          </form>
                           <form action="{{route('prestataire_force_destroy',$prest->id)}}" method="POST">
                              @csrf
                              <button type="submit" class="btn btn-secondary" style="margin:0px, 0px, 20px, 20px;">Suprimé 
                                   <!--i class="fa fa-close"></!--i-->
                              </button>
                          </form>
